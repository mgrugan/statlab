...
plt.show()`,
    ...
    { kind: "decode", title: "Why Jarque-Bera is built the way it is",
      body: "Nothing in this statistic is arbitrary. Each piece is answering a specific design question, and knowing which one turns a memorized formula into something you can reconstruct.",
      formula: "T=\\frac{n}{6}\\left(\\underbrace{\\widehat\\gamma_1^{\\,2}}_{\\text{skew off?}}+\\underbrace{\\frac{(\\widehat\\gamma_2-3)^2}{4}}_{\\text{tails off?}}\\right)\\ \\dot\\sim\\ \\chi^2_2",
      parts: [
        ["\\widehat\\gamma_1^{\\,2}", "distance from symmetry",
         "A normal has skewness $0$. Squaring means a left-skew of $-0.8$ is penalized exactly as hard as a right-skew of $+0.8$ — the test asks *how far from normal*, not *in which direction*."],
        ["(\\widehat\\gamma_2-3)^2", "distance from normal tails",
         "$3$ is not a magic number: it is **the normal's own kurtosis**. Writing $\\widehat\\gamma_2-3$ recentres the scale so that zero means “exactly as tail-heavy as a bell curve.” Financial returns routinely give $\\widehat\\gamma_2$ of 6–10, and squaring makes that enormous."],
        ["\\text{the } +", "why it needs **both**",
         "A distribution can be perfectly symmetric and still wildly non-normal if the tails are fat — the $t$ distribution is exactly that. Testing skewness alone would clear it. The sum means you have to pass **both** checks."],
        ["n/6", "why big samples reject so easily",
         "The statistic scales **linearly with sample size**. A fixed departure from normality that gives $T=4$ at $n=300$ gives $T=40$ at $n=3000$. With 20 years of daily data, any real departure at all is detected — which is why $T$ in the thousands is normal for returns and not a sign of a bug."],
        ["\\chi^2_2", "why 2 degrees of freedom",
         "You measured two things — skewness and kurtosis — so you spend two degrees of freedom. The 5% critical value is $5.99$; it does **not** grow with $n$, while $T$ does. That asymmetry is the whole story of large-sample testing."],
      ],
      reading: "“How far is this sample from a bell curve in shape, scaled up by how much evidence I have?”\n\n**The practical consequence you should be able to state.** Because $n$ multiplies the whole thing, a significant Jarque-Bera on 5000 returns tells you *that* the data are non-normal but nothing about *how badly*. For that you need the numbers themselves — a kurtosis of $4$ and a kurtosis of $30$ both give p-values of $0.000$ and mean completely different things for pricing an option." },
...
/* ===================================================================== */
{
  ...
  blocks: [
    ...
    { kind: "decode", title: "The AR(1) equation, read as a sentence",
      body: "Three symbols, three completely different jobs. Once you can say what each does, every AR(1) exam question is answerable without algebra.",
      formula: "X_t=\\mu+\\underbrace{\\phi}_{\\text{memory}}\\underbrace{(X_{t-1}-\\mu)}_{\\text{how far off you were}}+\\underbrace{\\epsilon_t}_{\\text{today's news}}",
      parts: [
        ["X_{t-1}-\\mu", "the current deviation, not the current level",
         "The model is written in terms of **distance from the long-run mean**, which is why $\\mu$ appears twice. If you were right on the mean yesterday, yesterday contributes nothing today."],
        ["\\phi", "what fraction of that deviation survives",
         "$\\phi=0.9$: 90% of yesterday's gap carries over, and it decays as $0.9,0.81,0.73,\\dots$ — slow, persistent, strongly autocorrelated. $\\phi=0.1$: gone almost immediately. $\\phi=0$: white noise, no memory at all."],
        ["|\\phi|<1", "the stationarity condition, and why it is that",
         "Each step multiplies the old deviation by $\\phi$. Shrink it every time and shocks die out, so the variance settles down. At $|\\phi|=1$ nothing decays — that is a random walk, variance grows without limit, and there is no mean to revert to. Above $1$ it explodes."],
        ["\\phi<0", "alternating, not drifting",
         "A negative $\\phi$ flips the sign each period: above the mean today, below tomorrow. Still mean-reverting, still stationary — stationarity is about $|\\phi|$, not its sign."],
        ["\\epsilon_t", "the part no model can see",
         "Mean-zero white noise. It is what keeps the series alive: without it $X_t$ would glide smoothly to $\\mu$ and stay there forever."],
      ],
      reading: "“Start from the average, add back a fraction of however wrong you were last time, then add fresh news.”\n\n**The confusion this clears up.** Students often treat *mean reversion* and *autocorrelation* as opposites — one pulls you back, the other says you keep going. An AR(1) with $\\phi=0.9$ has both, strongly, and they are the same fact seen twice: the series is pulled toward $\\mu$ **slowly**, and “slowly” is exactly what high autocorrelation means.\n\n**Contrast with GBM (Module 3), which is the exam's favourite comparison.** GBM shocks are permanent — the process wanders forever. AR(1) shocks decay geometrically — the process always comes home. That is the difference between a non-stationary and a stationary series." },

    { kind: "proof", title: "Where the AR(1) autocorrelation comes from",
      body: "The result you use when reading an ACF plot: an AR(1)'s autocorrelation decays **geometrically**, and the decay rate is $\\phi$ itself.",
      claim: "\\gamma_X(h)=\\frac{\\sigma_\\epsilon^2\\,\\phi^{|h|}}{1-\\phi^2},\\qquad \\rho_X(h)=\\phi^{|h|}",
      setup: "Take $\\mu=0$ without loss of generality (just re-centre), so $X_t=\\phi X_{t-1}+\\epsilon_t$ with $\\{\\epsilon_t\\}$ mean-zero white noise of variance $\\sigma_\\epsilon^2$, and assume $|\\phi|<1$ so a stationary solution exists.",
      steps: [
        ["X_t=\\sum_{k=0}^{\\infty}\\phi^{k}\\epsilon_{t-k}", "iterate the recursion; converges because $|\\phi|<1$"],
        ["\\gamma_X(0)=\\Var(X_t)=\\sigma_\\epsilon^2\\sum_{k=0}^{\\infty}\\phi^{2k}=\\frac{\\sigma_\\epsilon^2}{1-\\phi^2}", "independent $\\epsilon$'s, then a geometric series"],
        ["\\gamma_X(h)=\\Cov(X_{t+h},X_t)=\\Cov\\big(\\phi^{h}X_t+\\textstyle\\sum_{k=0}^{h-1}\\phi^{k}\\epsilon_{t+h-k},\\,X_t\\big)", "unroll $h$ steps forward"],
        ["=\\phi^{h}\\Var(X_t)", "the new shocks $\\epsilon_{t+1},\\dots,\\epsilon_{t+h}$ are independent of $X_t$"],
        ["\\rho_X(h)=\\frac{\\gamma_X(h)}{\\gamma_X(0)}=\\phi^{h}\\qquad\\blacksquare", "divide; symmetry gives $\\phi^{|h|}$ for $h<0$"],
      ],
      after: "**What to take from it.** The ACF of an AR(1) is a clean exponential decay with rate $\\phi$ — so when you look at an ACF plot and see bars shrinking by a roughly constant *factor*, you are looking at AR-type memory, and you can read $\\phi$ straight off the lag-1 bar.\n\nNote $\\Var(X_t)=\\sigma_\\epsilon^2/(1-\\phi^2)$ blows up as $|\\phi|\\to1$. That is the same $|\\phi|<1$ condition arriving from a different direction: without it, there is no finite variance for the process to be stationary around." },
...
plt.show()`,
    ...
    { kind: "decode", title: "Which test answers which question",
      body: "Three tests, three nulls, and they get mixed up constantly because two of them read backwards. Sort them once by asking **what is $H_0$ and what would rejecting it mean**, and the exam questions stop being tricky.",
      formula: "\\text{ADF}: H_0=\\text{unit root} \\quad\\big|\\quad \\text{Ljung-Box}: H_0=\\text{no autocorrelation} \\quad\\big|\\quad \\text{Jarque-Bera}: H_0=\\text{normal}",
      parts: [
        ["\\text{ADF},\\ p<0.05", "**evidence the series IS stationary**",
         "Backwards from every other test here. The null is the *bad* case (a unit root, i.e. non-stationary), so a **small** p-value is the **good** news. Small p $\\Rightarrow$ stationary."],
        ["\\text{ADF},\\ p=0.32", "**no evidence either way**",
         "You failed to reject a unit root. That is **not** proof of non-stationarity — failing to reject never establishes a null. The only honest sentence is “we found no evidence that the series is stationary.” This exact phrasing is worth rehearsing."],
        ["\\text{Ljung-Box},\\ p<0.05", "**evidence of autocorrelation somewhere in lags $1..H$**",
         "Note *somewhere*. It is a portmanteau test — it pools $H$ lags into one statistic, so it tells you something is correlated but not which lag. For that you read the ACF plot."],
        ["\\text{on } X_t \\text{ vs } X_t^2", "the same test, two very different meanings",
         "Ljung-Box on **returns** probes market efficiency — can you predict direction? Ljung-Box on **squared returns** probes volatility clustering — can you predict turbulence? On real data the first is borderline and the second is overwhelming. That contrast is the entire motivation for Part 4."],
        ["\\text{Jarque-Bera},\\ p<0.05", "**evidence the data are NOT normal**",
         "Same direction as Ljung-Box: small p means the null fails, and here the null was the comfortable assumption. This is the test that convicts Black-Scholes."],
      ],
      reading: "The rule that covers all three: **a small p-value always means “reject the null”** — so before you interpret anything, say what the null *was*. For ADF the null is the pessimistic case; for the other two it is the optimistic one.\n\n**The trap that catches people twice.** Ljung-Box on Amgen returns gives small p-values, which sounds like a violation of market efficiency. It is not, because *statistically significant* and *practically exploitable* are different things: with thousands of observations you can detect correlations far too small to trade through transaction costs. Real, detectable, and useless — keep that phrase ready." },
...
arch_series = simulate_arch(omega, alpha, 200)`,
    ...
    { kind: "decode", title: "GARCH(1,1) as a weather forecast",
      body: "Almost every GARCH used in practice is GARCH(1,1), and it is best read as a forecasting rule rather than an equation. Written out:",
      formula: "\\sigma_t^2=\\underbrace{\\omega}_{\\text{baseline}}+\\underbrace{\\alpha X_{t-1}^2}_{\\text{what happened}}+\\underbrace{\\beta\\sigma_{t-1}^2}_{\\text{what you already thought}}",
      parts: [
        ["\\beta\\sigma_{t-1}^2", "yesterday's forecast, carried forward",
         "Typically $\\beta\\approx0.9$ in equity data. You keep 90% of what you believed yesterday — this is the **inertia** term, and it is why fitted volatility looks smooth rather than jumpy."],
        ["\\alpha X_{t-1}^2", "yesterday's surprise, as a correction",
         "Typically $\\alpha\\approx0.05$. A big move nudges the forecast up; a quiet day nudges it down. Small weight, because a single day's return is a **noisy** read on volatility — which is exactly why you need the $\\beta$ term to do the remembering."],
        ["\\omega", "the floor nothing can push you below",
         "Volatility never decays to zero, because $\\omega>0$ is always added back. It sets the level the forecast drifts toward once shocks fade."],
        ["\\alpha+\\beta", "how long the market remembers",
         "The fraction of a volatility shock still present one day later. At $0.95$, a shock has half-decayed after about **14 days** ($0.95^{14}\\approx0.49$); at $0.99$, about **69 days**. This single number is the whole personality of a fitted GARCH."],
        ["\\alpha+\\beta<1", "the stationarity condition",
         "Each day multiplies the leftover shock by $\\alpha+\\beta$. Below 1 and shocks fade, so there is a finite long-run variance to revert to. At exactly 1 (IGARCH) they never fade and the unconditional variance does not exist — fitted values just above 1 are a red flag worth reporting."],
        ["X_{t-1}^2", "the sign is gone",
         "Squaring means $-5\\%$ and $+5\\%$ produce **identical** forecasts. That is the model's one structural blind spot, and it is precisely the leverage effect of Module 11 — the reason APARCH, EGARCH and GJR-GARCH exist."],
      ],
      reading: "“Tomorrow's volatility is mostly today's volatility, nudged by how surprising today was, and pulled gently toward a long-run level.”\n\n**Why this is the model that won.** Volatility clustering is precisely the statement that *volatility is predictable from its own past* — and this is the simplest equation that says so. Three parameters buy you a forecast that adapts fast (through $\\alpha$), remembers long (through $\\beta$), and never runs away (through $\\alpha+\\beta<1$).\n\n**And what it does not do:** GARCH models the **variance**, not the direction. Knowing tomorrow will be turbulent tells you nothing about which way. That is entirely consistent with market efficiency — which is why a model this useful is not a money machine." },

    { kind: "proof", title: "Where $\\alpha+\\beta<1$ comes from",
      body: "The condition is not a convention — it falls out of asking for a finite long-run variance. Two lines, and it also hands you the formula for that variance.",
      claim: "\\alpha+\\beta<1\\ \\Longleftrightarrow\\ \\Var(X_t)=\\frac{\\omega}{1-\\alpha-\\beta}<\\infty",
      setup: "Take GARCH(1,1): $X_t=\\sigma_t\\epsilon_t$ with $\\epsilon_t$ iid, mean $0$, variance $1$, independent of the past. Suppose the process is stationary and write $v=\\E(\\sigma_t^2)$, the same at every $t$.",
      steps: [
        ["\\E(X_{t-1}^2)=\\E(\\sigma_{t-1}^2\\epsilon_{t-1}^2)=\\E(\\sigma_{t-1}^2)\\E(\\epsilon_{t-1}^2)=v", "independence, and $\\E(\\epsilon^2)=1$"],
        ["v=\\E(\\sigma_t^2)=\\omega+\\alpha\\E(X_{t-1}^2)+\\beta\\E(\\sigma_{t-1}^2)", "take expectations of the GARCH recursion"],
        ["v=\\omega+\\alpha v+\\beta v", "substitute the first line and stationarity"],
        ["v(1-\\alpha-\\beta)=\\omega", "collect terms"],
        ["v=\\frac{\\omega}{1-\\alpha-\\beta}\\qquad\\blacksquare", "solve; positive and finite only if $\\alpha+\\beta<1$"],
      ],
      after: "**Read the answer, not just the algebra.** $\\omega/(1-\\alpha-\\beta)$ is the level volatility returns to when nothing is happening — the “normal” variance of the asset. As $\\alpha+\\beta\\to1$ the denominator goes to zero and that level runs off to infinity: the process has so much memory that it never settles, so asking for a long-run average is meaningless.\n\n**The exam version.** Given a fitted model with $\\omega=0.00001$, $\\alpha=0.09$, $\\beta=0.90$: the sum is $0.99<1$, so it is stationary, and the long-run variance is $0.00001/0.01=0.001$, i.e. a daily volatility of about $3.2\\%$ — roughly $50\\%$ annualized. Checking that the number is plausible is a good habit; an implied annual volatility of 900% means an arithmetic slip." },
...
/* ===================================================================== */
{
  ...
  blocks: [
    ...
    { kind: "decode", title: "Reading Black-Scholes one input at a time",
      ...
      formula: "C=\\underbrace{P_0e^{(\\nu+\\sigma^2/2)t}}_{\\E(P_t)}\\,\\Phi\\!\\underbrace{\\left(\\frac{M+\\nu t+\\sigma^2 t}{\\sigma\\sqrt{t}}\\right)}_{d_1}-\\;K\\,\\Phi\\!\\underbrace{\\left(\\frac{M+\\nu t}{\\sigma\\sqrt{t}}\\right)}_{d_2}",
      parts: [
        ["M=\\log(P_0/K)", "moneyness \u2014 how far in the money you start",
         "The **only** place $P_0$ and $K$ meet. It is positive when the spot is above the strike, zero at the money, negative below. Because it is a *ratio* inside a log, doubling both the spot and the strike leaves $M$ unchanged \u2014 the formula cares about proportion, not dollars."],
        ["\\nu t", "where the drift carries you",
         "Expected log growth over the life of the option. It shifts the whole distribution of $\\log P_t$ right, which raises both $\\Phi$ arguments: more drift, more likely to finish in the money, higher price."],
        ["\\sigma\\sqrt{t}", "how far the price can wander",
         "The denominator of both $d$'s. It converts a distance in log-price into a distance in standard deviations. **This is the only channel through which time enters as risk** \u2014 and it enters as $\\sqrt{t}$, not $t$, which is why a 4-year option is only twice as uncertain as a 1-year one."],
        ["\\Phi(d_2)", "the chance you exercise",
         "Read the argument as \u201Chow many SDs above the strike do I expect to land?\u201D Big and positive \u2192 near-certain exercise \u2192 $\\Phi\\to1$. Big and negative \u2192 $\\Phi\\to0$ and the option is nearly worthless."],
        ["+\\sigma^2t", "the extra term in $d_1$ only",
         "The single most-missed detail. $d_1=d_2+\\sigma\\sqrt{t}$ \u2014 exactly one standard deviation apart. It is there because the first term weights outcomes by the **price**, and expensive outcomes are the high ones. Nothing more mysterious than that."],
        ["\\sigma\\uparrow", "volatility always raises a call's value",
         "Raising $\\sigma$ pushes the $\\E(P_t)$ factor up *and* widens the distribution. The downside is capped at losing the premium, the upside is not \u2014 so more uncertainty is unambiguously good for the holder. This is the payoff asymmetry from Module 1, now visible in an equation."],
      ],
      reading: "\u201CStart from what I expect the stock to be worth, keep only the futures where I'd exercise, and subtract the strike I'd have to pay in those same futures.\u201D\n\n**The exam-ready summary of the comparative statics:** a call is worth more when $P_0$ is higher, $K$ is lower, $t$ is longer, $\\nu$ is higher, or $\\sigma$ is higher. The first four are obvious. The fifth \u2014 *more risk, more value* \u2014 is the one that sounds wrong and is the whole point of owning an option.\n\n**And the honest caveat:** three of these five inputs are facts ($P_0$, $K$, $t$); two are opinions ($\\nu$, $\\sigma$). The formula is a machine for converting your beliefs about drift and volatility into a price. It does not know anything the statistics of Parts 2-4 do not tell it." },

    { kind: "decode", title: "Where the formula stops being true",
      body: "You should be able to say which assumption each of Modules 5-10 attacks. That is what makes the second half of this course a critique of the first half rather than a different subject.",
      formula: "\\log P_t\\sim N\\big(\\log P_0+\\nu t,\\ \\sigma^2 t\\big)",
      parts: [
        ["N(\\cdot,\\cdot)", "\u201Cthe log price is normal\u201D",
         "**Broken.** QQ plots, excess kurtosis and Jarque-Bera all reject it decisively (Modules 5-6). Real log returns are fat-tailed, so Black-Scholes under-prices far out-of-the-money options \u2014 the crashes it treats as impossible do happen."],
        ["\\sigma^2 t", "\u201Cvolatility is one constant number\u201D",
         "**Broken.** Volatility clusters and is itself a time series (Modules 9-10). That is exactly what ARCH and GARCH were invented to model, and it is why the market quotes a different implied $\\sigma$ at every strike \u2014 the volatility smile."],
        ["\\text{independent increments}", "\u201Cyesterday tells you nothing about today\u201D",
         "**Half broken.** Returns themselves are near-uncorrelated, so the ACF of $X_t$ looks fine. But the ACF of $X_t^2$ does not (Module 8). Uncorrelated is not independent, and the difference is where all the money is."],
        ["\\nu t\\ \\text{symmetric}", "\u201Cup moves and down moves are alike\u201D",
         "**Broken.** The leverage effect says volatility rises more after a fall than after an equal-sized rise. Standard GARCH inherits this blind spot, since squaring erases the sign \u2014 hence APARCH, EGARCH and GJR-GARCH."],
      ],
      reading: "Black-Scholes is not wrong so much as **too smooth**. Every departure the second half of the course measures \u2014 fat tails, clustering, asymmetry \u2014 is a way the real world is rougher than a single normal distribution with one fixed $\\sigma$.\n\nThat is also the honest answer to \u201Cwhy do practitioners still use it?\u201D They do not use it to *predict* prices. They use it as a **common language**: quoting a price as an implied volatility is a way of saying where you disagree with the model, and everyone knows which model." },
